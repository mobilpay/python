...
class InvoiceItem:
    ...
    def load_from_xml(self, item_node):
        ...
        if len(xml_temp) != 1:
            raise Exception("load_from_xml -> Invalid code element" + str(self.ERROR_LOAD_FROM_XML_CODE_ELEM_MISSING))
        self._code = unquote(xml_temp[0].firstChild.nodeValue, encoding="utf-8")

        xml_temp = item_node.getElementsByTagName("name")
        if len(xml_temp) != 1:
            raise Exception("load_from_xml -> Invalid name element" + str(self.ERROR_LOAD_FROM_XML_NAME_ELEM_MISSING))
        ...
        if len(xml_temp) != 1:
            raise Exception(
                "load_from_xml -> Invalid quantity element" + str(self.ERROR_LOAD_FROM_XML_QUANTITY_ELEM_MISSING))

        self._quantity = Decimal(unquote(xml_temp[0].firstChild.nodeValue, encoding="UTF-8"))

        if self._quantity <= 0:
            raise Exception("load_from_xml -> Invalid quantity value: " + str(self._quantity) + " " + str(
                self.ERROR_LOAD_FROM_XML_QUANTITY_ELEM_EMPTY))

        xml_temp = item_node.getElementsByTagName("price")
        if len(xml_temp) != 1:
            raise Exception("load_from_xml -> Invalid price element" + str(self.ERROR_LOAD_FROM_XML_PRICE_ELEM_MISSING))

        self._price = Decimal(unquote(xml_temp[0].firstChild.nodeValue, encoding="UTF-8"))

        if self._price < 0:
            raise Exception("load_from_xml -> Invalid quantity value: " + str(self._price) + " " + str(
                self.ERROR_LOAD_FROM_XML_PRICE_ELEM_EMPTY))

        xml_temp = item_node.getElementsByTagName("vat")

        if len(xml_temp) != 1:
            raise Exception("load_from_xml -> Invalid vat element" + str(self.ERROR_LOAD_FROM_XML_VAT_ELEM_MISSING))
